...
namespace Dzik.crypto.protocols
{
    ...
    internal static class MasterKeysPacker
    {
        ...
        internal static byte[] PackKeys(byte[] masterA, byte[] masterB, byte[] masterC, byte[] hmacKey)
        {
            if (masterA.Length != keyLenBytes || masterB.Length != keyLenBytes || masterC.Length != keyLenBytes || hmacKey.Length != authKeyLenBytes)
            {
                throw new Exception($"Incorrect key length! Expected {keyLenBytes * 8} but got a different length on at least one of the keys");
            }

            var allKeys = new byte[masterA.Length + masterB.Length + masterC.Length + hmacKey.Length];
        ...
        {
            var (masterA, masterB, masterC, hmacKey) = UnpackKeysIntoNArrays(allKeys);

            try
            {
                return new KeysVault(masterA, masterB, masterC, hmacKey);
            }
            catch (Exception) { }
            finally
            {
        ...
        internal static string GetStringRepresentation(byte[] allKeys)
        {
            ...
            var (masterA, masterB, masterC, hmacKey) = UnpackKeysIntoNArrays(pinnedAllKeys);
        ...
        private static (PinnedBytes, PinnedBytes, PinnedBytes, PinnedBytes) UnpackKeysIntoNArrays(PinnedBytes allKeys)
        {
            ...
            Array.Copy(allKeys.bytes, 3 * keyLenBytes, hmacKey.bytes, 0, authKeyLenBytes);

            return (masterA, masterB, masterC, hmacKey);
        }
    }
}
